...
export function KeyframesScreen() {
  ...
  const addKeyframe = () => {
    const lastKeyframe = [...config.keyframes]
      .sort((a, b) => a.percentage - b.percentage)
      .pop();
    const newPercentage = lastKeyframe
      ? Math.min(lastKeyframe.percentage + 25, 100)
      : 50;
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        <div
          ...
          style={{
            ...
            animation: config.isPlaying
              ? `${config.name}-preview ${config.duration}ms ${config.timingFunction} ${config.delay}ms ${config.iterationCount} ${config.direction} ${config.fillMode}`
              : "none",
          }}
        >
          Preview
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setConfig((prev) => ({
                  ...prev,
                  fillMode: e.target.value as AnimationConfig["fillMode"],
                }))
              }
              className="w-full px-3 py-2 text-sm border border-border rounded focus:outline-none focus:ring-2 focus:ring-accent/20"
            >
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-muted">
            {config.keyframes.length} keyframes
          </span>
        </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium">
                  Keyframe {index + 1} - {keyframe.percentage}%
                </span>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateKeyframe(keyframe.id, {
                        translateY: parseInt(e.target.value) || 0,
                      })
                    }
                    className="text-sm"
                  />
