...
export function CaseConverterScreen() {
  ...
  const resetTool = () => {
    ...
    setConfig(prev => ({ ...prev, input: defaultInput, preserveLineBreaks: true }));
...
tOGGLE cASE: ${config.outputs.toggleCase}`;
  ...
  const caseFormats = [
    { key: 'camelCase' as const, name: 'camelCase', description: 'JavaScript variables, object properties' },
    { key: 'pascalCase' as const, name: 'PascalCase', description: 'Class names, constructors' },
    { key: 'snakeCase' as const, name: 'snake_case', description: 'Python variables, database fields' },
    { key: 'kebabCase' as const, name: 'kebab-case', description: 'CSS classes, HTML attributes' },
    { key: 'constantCase' as const, name: 'CONSTANT_CASE', description: 'Constants, environment variables' },
    { key: 'dotCase' as const, name: 'dot.case', description: 'Configuration keys, namespaces' },
    { key: 'pathCase' as const, name: 'path/case', description: 'File paths, routes' },
    { key: 'sentenceCase' as const, name: 'Sentence case', description: 'Regular sentences, descriptions' },
    { key: 'titleCase' as const, name: 'Title Case', description: 'Headings, titles' },
    { key: 'lowerCase' as const, name: 'lowercase', description: 'General text, simple processing' },
    { key: 'upperCase' as const, name: 'UPPERCASE', description: 'Emphasis, constants' },
    { key: 'toggleCase' as const, name: 'tOGGLE cASE', description: 'Stylistic effects, testing' }
  ];
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-auto">
          <div className="grid grid-cols-1 gap-2 text-xs">
            <div><strong>camelCase:</strong> <span className="font-mono">{config.outputs.camelCase}</span></div>
            <div><strong>snake_case:</strong> <span className="font-mono">{config.outputs.snakeCase}</span></div>
            <div><strong>kebab-case:</strong> <span className="font-mono">{config.outputs.kebabCase}</span></div>
            <div><strong>CONSTANT_CASE:</strong> <span className="font-mono">{config.outputs.constantCase}</span></div>
            <div><strong>Title Case:</strong> <span className="font-mono">{config.outputs.titleCase}</span></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => setConfig(prev => ({ ...prev, preserveLineBreaks: e.target.checked }))}
            className="rounded"
          />
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 gap-3">
          {caseFormats.map((format) => (
            <div key={format.key} className="p-3 bg-surface-1 border border-border rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Type size={14} className="text-accent" />
                  <span className="text-sm font-medium">{format.name}</span>
                  <span className="text-xs text-muted">({format.description})</span>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="bg-gray-50 rounded p-2 font-mono text-sm">
                {config.outputs[format.key] || (
                  <span className="text-muted italic">Enter text to see conversion...</span>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-3 bg-surface-1 border border-border rounded-lg">
            ...
            <ul className="text-xs space-y-1 text-muted">
              <li><strong>kebab-case:</strong> CSS classes, HTML attributes</li>
              <li><strong>dot.case:</strong> Configuration keys</li>
              <li><strong>path/case:</strong> URLs, file paths</li>
              <li><strong>Title Case:</strong> Headings, labels</li>
            </ul>
          </div>
        </div>
      </div>
