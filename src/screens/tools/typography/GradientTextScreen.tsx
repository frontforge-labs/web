...
export function GradientTextScreen() {
  ...
  const addGradientStop = () => {
    const newPosition = config.stops.length > 0 ?
      Math.round((config.stops[config.stops.length - 1].position + 100) / 2) : 50;
    ...
    >
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Fallback Color</label>
            <div className="flex items-center gap-2">
              <input
                type="color"
                value={config.fallbackColor}
                onChange={(e) => setConfig(prev => ({ ...prev, fallbackColor: e.target.value }))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setConfig(prev => ({ ...prev, gradientType: e.target.value as 'linear' | 'radial' }))}
              className="w-full px-3 py-2 text-sm border border-border rounded focus:outline-none focus:ring-2 focus:ring-accent/20"
            >
          ...
          {config.gradientType === 'linear' && (
            <div>
              <label className="block text-sm font-medium mb-2">Direction (degrees)</label>
              <Input
                type="number"
                value={config.direction}
                onChange={(e) => setConfig(prev => ({ ...prev, direction: parseInt(e.target.value) || 45 }))}
                min={0}
                max={360}
              />
            </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => setConfig(prev => ({ ...prev, backgroundClip: e.target.checked }))}
              className="rounded"
            />
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-muted">{config.stops.length} color stops</span>
        </div>
            ...
            {config.stops.map((stop) => (
              <div
                ...
                onClick={() => {
                  ...
                  const input = document.querySelector(`input[data-stop-id="${stop.id}"]`) as HTMLInputElement;
        ...
        <div className="space-y-3">
          {config.stops.map((stop, index) => (
            <div key={stop.id} className="p-3 bg-surface-1 border border-border rounded-lg">
      ...
      <div className="mb-6">
        ...
        <div className="p-3 bg-surface-1 border border-border rounded-lg text-sm">
          ...
          <p className="mt-2 text-xs text-muted">
            Always include a fallback color for browsers that don't support background-clip: text
          </p>
        </div>
      </div>
    </ToolContainer>
